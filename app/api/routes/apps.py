...
from app.schemas.apps import ConnectedAppCreate, ConnectedAppResponse
...
def create_app(
    ...
    exists = db.query(ConnectedApp).filter(ConnectedApp.app_id == payload.app_id).first()
    if exists:
        raise HTTPException(status_code=400, detail="app_id already exists")
